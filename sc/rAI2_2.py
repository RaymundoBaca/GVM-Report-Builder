...
def main(pdf_path, txt_path, log, output_dir, progress_callback=None):
    ...
    for i in range(1, len(host_blocks), 2):
        if progress_callback and total_hosts > 0:
            progress_callback(0.05 + 0.45 * (i // 2 + 1) / total_hosts)
        ...
        for v in vulnerabilities[1:]:
            ...
            summary_match = re.search(
                r"Summary:\s*(.*?)(?=\n(?:Vulnerability Detection Result:|Impact:|Solution:|Details:|Vulnerability Insight:|Affected Software/OS:|\n))",
                v, re.DOTALL
            )
            solution_match = re.search(
                r"Solution:\s*(.*?)(?=\n(?:Vulnerability Detection Result:|Impact:|Summary:|Details:|Vulnerability Insight:|Affected Software/OS:|\n))",
                v, re.DOTALL
            )
            impact_match = re.search(
                r"Impact:\s*(.*?)(?=\n(?:Vulnerability Detection Result:|Solution:|Summary:|Details:|Vulnerability Insight:|Affected Software/OS:|\n))",
                v, re.DOTALL
            )
            vdr_match = re.search(
                r"Vulnerability Detection Result:\s*(.*?)(?=\n(?:Impact:|Solution:|Summary:|Details:|Vulnerability Insight:|Affected Software/OS:|\n))",
                v, re.DOTALL
            )
            ...
            vdr = clean_text(vdr_match.group(1) if vdr_match else "")
    ...
    for i in range(1, len(host_blocks), 2):
        ...
        for v in vulnerabilities[1:]:
            ...
            if vuln_details[nvt]["desc"] != "DESCRIBED":
                ...
                if vuln_details[nvt]["vdr"]:
                    output.append(f"Verification:\n{vuln_details[nvt]['vdr']}")
